...
@Suite("Desfecho da verificação e tabela de gravação")
struct VerificationOutcomeTests {
    ...
    @Test("os sete desfechos da verificação existem, e nenhum outro")
    ...
    @Test("cada desfecho decide gravar ou não, sem caso omisso")
    ...
    @Test("sucesso grava, porque há evidência de que funciona")
    func successPersists() {
        #expect(VerificationOutcome.success.shouldPersist)
    }

    @Test("credencial recusada e credencial expirada não gravam")
    ...
    @Test("credencial ilegível não é desfecho desta verificação, e o que não tem evidência não grava")
